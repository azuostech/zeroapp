...
async function revertIdentityCoins({ supabase, userId, declarationId }) {
  const { data: reversalTx, error: reversalError } = await supabase
    .from('coins_transactions')
    .select('id')
    .eq('user_id', userId)
    .eq('action_type', 'identity_removed')
    .contains('metadata', { declaration_id: declarationId })
    .limit(1)
    .maybeSingle();
  ...
  const { data: awardTx, error: awardError } = await supabase
    ...
    .select('id,amount')
    .eq('user_id', userId)
    .eq('action_type', 'identity_registered')
    .contains('metadata', { declaration_id: declarationId })
    .gt('amount', 0)
  ...
  if (amountToRevert > 0) {
    const { error: txError } = await supabase.from('coins_transactions').insert({
      ...
      action_type: 'identity_removed',
      description: 'Estorno por remoção de identidade',
      metadata: {
...
export async function DELETE(request, { params }) {
  ...
  if (findError) {
    return NextResponse.json({ error: findError.message || 'Erro ao localizar declaração' }, { status: 500 });
  }
  ...
  let balance = null;
  try {
    balance = await revertIdentityCoins({
      supabase,
  ...
  if (error) {
    return NextResponse.json({ error: error.message || 'Erro ao remover declaração' }, { status: 500 });
  }
